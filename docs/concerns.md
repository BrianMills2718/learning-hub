...
| H-007 | mitigated | Uncompressed build warnings overstated the fresh-load diagnosis. A production browser trace measured Godel's home route at about 326 kB of JavaScript after splitting route-specific views; its 46 kB-compressed node-detail chunk now loads only on a lesson route. Second Brain's 776 kB migration chunk remains lazy, while its fresh home route is about 391 kB. | Measure mobile transfer and route timing on the hosted build before further splitting; only split confirmed hot-path modules. |
